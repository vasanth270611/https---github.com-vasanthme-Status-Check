...
function EnvironmentTable() {
  ...
  const handleSubmit = (event) => {
    ...
    const { id, name, description } = environment;
    dispatch(
      addEnvironment(selectedEnv, {
        id: id,
        name: name,
  ...
  //  setEnvironment({ ...environment, [fieldName]: value});
  // }
  ...
  const handleDelete = (id) => {
    dispatch(deleteEnvironment(selectedEnv, id));
  };
  ...
  const onChangeEnv = (event) => {
    ...
    dispatch(getEnvironment(value));
  };

  const columns = [
    { field: "id", headerName: "Environment Id", width: 150 },
    { field: "name", headerName: "Environment Name", width: 250 },
  ...
  return (
    <>
      <Grid container>
        <Grid item xs={6}>
          <Typography variant="h6">Environment List</Typography>
        </Grid>
